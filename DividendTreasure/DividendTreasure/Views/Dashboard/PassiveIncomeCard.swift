...
struct PassiveIncomeCard: View {
    ...
    var body: some View {
        PassiveIncomeCardContent(
            targetAmount: targetAmount,
            currentAmount: currentAmount,
            showInlineChevron: false
        )
...
/// 被动收入卡片的实际内容。PassiveIncomeCard 与 ClickablePassiveIncomeCard 共用，
/// 避免之前两处 ~100 行的复制粘贴（任何修改只需改一处）。
private struct PassiveIncomeCardContent: View {
    let targetAmount: Double
    let currentAmount: Double
    let showInlineChevron: Bool
